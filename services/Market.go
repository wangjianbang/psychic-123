...
// 根据账户ID查询订单完成情况  by wangjb
// strOrderID: 订单ID
// return: string字符串  尚未完成
func GetOrderStatus(strOrderID string) models.OrderStatusReturn {
	orderstatusReturn := models.OrderStatusReturn{}

	strRequest := fmt.Sprintf("/v1/order/orders/%s", strOrderID)
	jsonBanlanceReturn := untils.ApiKeyGet(make(map[string]string), strRequest)
	json.Unmarshal([]byte(jsonBanlanceReturn), &orderstatusReturn)

	return orderstatusReturn
}
